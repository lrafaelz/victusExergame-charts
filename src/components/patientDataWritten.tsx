...
export const PatientDataWritten: React.FC<PatientDataWrittenProps> = ({
  ...
  return (
    <Box key={index} sx={{ border: 1, borderColor: 'gray.200', borderRadius: 1, p: 4, my: 4 }}>
      ...
      <Typography sx={{ bgcolor: 'gray.200' }}>Pontuação: {session.pontuacao}</Typography>
      <p>Tempo de sessão: {session.tempoDeSessao}</p>
      <Typography sx={{ bgcolor: 'gray.200' }}>EMG: {session.EMG?.join(', ')}</Typography>
